...
function minerPayload() {
  return {
    ...
    sources: [{ label: "Technocore DID Starter", url: "https://github.com/zunmax/technocore-did-starter#-share-the-contribution-", note: "DID and contribution trail." }],
    mode: "readiness_only",
    overall_status: "blocked",
    readiness_score: 42,
    compute: { gpu_probe: gpuProbe, cpu_threads: 16, memory_total_gb: 64, memory_free_gb: 32, cuda_visible: false },
    economics: { base_self_stake: "10,000 FLOP plus capacity bond", reward_share: "75% miner pool plus sessions" },
    ...
    integration_path: [{ id: "calibrate", label: "GPU calibration", state: "manual_required", detail: "Measure throughput before advertising capacity.", source: "https://flop.finance/intro/miner/" }],
...
function validatorPayload() {
  return {
    ...
    sources: [{ label: "FLOP validator guide", url: "https://flop.finance/intro/validator/", note: "Stake, DA and slashing." }],
    mode: "readiness_only",
    overall_status: "blocked",
    readiness_score: 33,
    ...
    economics: { reward_share: "10% of block rewards", slash_risk: "severe slashing" },
    compute: { gpu_probe: gpuProbe, cpu_threads: 16, memory_total_gb: 64, memory_free_gb: 32, cuda_visible: false },
    lifecycle: [{ id: "register", label: "Register", state: "manual_required", detail: "Self-sign registration and freeze stake." }],
    integration_path: [{ id: "da", label: "Data availability", state: "blocked", detail: "Provision DA store-and-serve endpoint.", source: "https://flop.finance/intro/validator/" }],
...
describe("FlopOperatorPanel", () => {
  it("renders Miner readiness without operational side-effect controls", async () => {
    ...
    expect(screen.getByText("Integration Path")).toBeInTheDocument();
    expect(screen.getByText("GPU calibration")).toBeInTheDocument();
    expect(screen.getByText(/10,000 FLOP/)).toBeInTheDocument();
    expect(screen.getByText("Technocore DID Starter")).toBeInTheDocument();
  ...
  it("renders Validator readiness as manual/on-chain gated", async () => {
    ...
    expect(screen.getByText("FLOP validator guide")).toBeInTheDocument();
