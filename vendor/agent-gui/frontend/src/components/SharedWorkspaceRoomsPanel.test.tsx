...
function notFoundResponse() { return Promise.resolve({ ok: false, status: 404, statusText: "Not Found", json: async () => ({ detail: "not found" }) } as Response); }
...
describe("SharedWorkspaceRoomsPanel", () => {
  ...
  it("shows a Network setup notice instead of raw 404 text", async () => {
    ...
    expect(await screen.findByText("Shared room backend required")).toBeInTheDocument();
    expect(screen.getByText(/opens private-name team rooms/i)).toBeInTheDocument();
    expect(screen.getByTestId("shared-workspaces").textContent).not.toMatch(/404|Not Found/i);
  });
});
