...
The dashboard now splits top-level operations into Workspaces / Projects, Market, Miner, Validator, Work, Deals, Network, and Trust & Vault. Work contains only task/job operations plus Federated Workbench. Market is the guided FLOP session start: Request -> Match -> Prove -> Settle. Its safe primary action creates a local Workspaces request draft; Matchmaking remains recommendation-only underneath it, followed by Skill Registry and Skill Finder. Deals contains TCLK/PaperRail offer and dealbook flows. Network contains mailboxes, subtask delegation, shared workspace rooms, activity, and chat inspection. Trust & Vault combines reputation/review evidence with signing policy, Capability Registry, and delegation notes. Miner and Validator are routable through `/osa-network/#/miner` and `/osa-network/#/validator`; the server redirects `/miner` and `/validator` into those SPA-safe routes so static hosts do not expose 404s for top-level operator tabs. If a static deployment lacks the live OSA `/api/*` backend, Miner, Validator, Matchmaking, and Skill Registry render guided preview/setup models with safe next steps.
