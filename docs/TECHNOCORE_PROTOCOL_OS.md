...
3. Add node identity, per-agent DIDs, signed capability publication, registry scanning, delegation, signing policies, reputation projection, and skill finding. *(implemented: local standard and custom Agent Profiles publish canonical `osa-capability-registry/1` KV records; scanner keeps a restart-persistent verified/stale/untrusted projection in Vault; deterministic per-agent `osa-reputation/1` records publish accepted-result, verified-job-result, terminal-PaperRail, refund/dispute, and hashed-counterparty evidence into a separately verified Trust projection; Work provides deterministic local/federated capability search with exact-identity reputation context and local-only pending-desk selection.)*
...
| 3 – Per-agent DIDs, capabilities, delegation, policies | ✅ managed per-agent Ed25519 DIDs, signed Capability Registry, scanner projection, deterministic Skill Finder, local-only pending-desk selection, /api/agents/dids, signing policies, delegations |
