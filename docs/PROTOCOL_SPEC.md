...
`osa-flop-miner-console/1` and `osa-flop-validator-console/1` are dashboard readiness projections grounded in the FLOP Yellowpaper. Miner rows cover local compute/GPU visibility, Technocore/DID readiness, wallet/stake blockers, calibration/model-root status, lifecycle steps, Yellowpaper economics, cited integration sources, and explicit authority flags. Validator rows cover identity, stake-floor, consensus-client, DA, recent PoUI-work, attestation-quorum, lifecycle, validator economics, cited integration sources, and explicit authority flags. These schemas are not chain state, not settlement proofs, and not executable operator commands. They cannot lease GPUs, register miners or validators, bond stake, accept inference sessions, author blocks, sign validator attestations, publish DA, claim rewards, or move FLOP. Public browser payloads omit raw signatures, keys, seeds, connector tokens, private task bodies, and filesystem paths.
