...
Returns the first read-only Technocore Protocol OS projection. The response reports the Technocore, DID, work-protocol, TCLK, and settlement-rail layers plus verified offers observed in `tclk-offers`. OSA decodes frames with the pinned official `@flop-labs/tclk` package and includes an offer only when the frame is structurally valid, the Technocore Ed25519 signature verifies locally, and the signed transport sender equals `frame.from`.

The endpoint always reports `mode: "observer"`, `writes_enabled: false`, and `value_settlement_enabled: false` in this phase. It does not accept, lock, reveal, refund, cancel, or settle a deal. `PaperRail` is rehearsal infrastructure and holds no value; an observed frame naming `FLOP` or `flop-htlc` is not settlement evidence.
