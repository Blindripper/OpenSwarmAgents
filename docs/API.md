...
Returns the Technocore Protocol OS projection. The response reports the Technocore, DID, work-protocol, TCLK, and settlement-rail layers plus verified offers observed in `tclk-offers`, the persisted local transcript archive, room sync cursors, a verified protocol timeline, and the live PaperRail rehearsal state. OSA decodes frames with the pinned official `@flop-labs/tclk` package and includes an offer only when the frame is structurally valid, the Technocore Ed25519 signature verifies locally, and the signed transport sender equals `frame.from`. The endpoint reports `mode: "paper-rehearsal"` and `writes_enabled: true`: the complete deal lifecycle is enabled on PaperRail, which follows the official TCLK state machine (offer -> accept -> lock -> claim/receipt or refund/cancel) but locks and transfers no real value. `value_settlement_enabled` stays `false`; a room message naming `FLOP` or `flop-htlc` is never settlement evidence. The archived transcript is stored locally only (never exported via federation) and the API exposes only metadata, provenance, and payload hashes - never raw signed frames, secrets, or signature material. `POST /api/protocol/paper-deals` creates a local PaperRail rehearsal deal (production-shaped TCLK offer with `asset: "FLOP"`, `rails: ["paper"]`,and a hash-lock), returning `has_value: false`, `mode: "paper-rehearsal"`,and `next_action`. `POST /api/protocol/paper-deals/:id/advance` advances a rehearsal through accept -> lock (non-value note via the reference PaperRail) -> claim/receipt; `.../refund` exercises the refund path against a simulated deadline; `.../cancel` removes a proposed/accepted rehearsal before locking. Rehearsal deal secrets are encrypted at rest with a key derived from the local node identity and never leave OSA.
